...
import { replaceTags, addEllipsis } from '../../utils/string'
...
module.exports = class DocsDocsCommand extends Command {
  ...
  async run(msg, args) {
    ...
    try {
      const response = axios.get(MDN_SEARCH_URL + params)

      if (response.data.documents.length) {
        const doc = response.data.documents[0]
        const excerpt = addEllipsis(replaceTags(doc.excerpt)).replace('``', '')
